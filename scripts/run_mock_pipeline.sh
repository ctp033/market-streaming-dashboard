...
cleanup() {
  echo
  echo "Stopping mock market-data pipeline..."
...
echo "Starting mock Finnhub trade producer..."
"$PYTHON_BIN" src/producers/mock_finnhub_trade_producer.py &
PIDS+=("$!")
...
echo
echo "Mock pipeline is running."
echo "Dashboard: http://$STREAMLIT_HOST:$STREAMLIT_PORT"
echo "Select 'Mock pipeline' in the dashboard sidebar."
echo "Press Ctrl+C here to stop the mock producer, feature engine, and dashboard."
